...
def favorites(page=default_page):
    ...
    if user_id:
        ...
        # domyślne parametry wyszukiwania, sortowania
        default_dep_uri = ''
        default_note = 'desc'
        default_only_foto = 'yes'
        default_has_my_info = 'no'

        # Poprawne wartości dla parametrów
        corrects_search = ['yes', 'no']
        corrects_sort = ['asc', 'desc']

        dep = Departments(db.get_db(), Museum_api())
        corrests_uri = dep.get_departaments_uri()

        # Pobranie argumentów z paska
        dep_uri = request.args.get('dep_uri', default_dep_uri).lower()
        if dep_uri not in corrests_uri:
            dep_uri = default_dep_uri

        note = request.args.get('note', default_note).lower()
        if note not in corrects_sort:
            note = default_note

        only_foto = request.args.get('only_foto', default_only_foto).lower()
        if only_foto not in corrects_search:
            only_foto = default_only_foto

        has_my_info = request.args.get('has_my_info', default_has_my_info).lower()
        if has_my_info not in corrects_search:
            has_my_info = default_has_my_info

        #Pobranie id produktów zalogowanego uzytkownika wg filtrów w pasku adresowym
        arts = Arts(db.get_db(), Museum_api())
        department_id = dep.get_department_id_from_uri(dep_uri)
        arts_id = arts.get_user_arts_by_param(user_id, department_id, note, only_foto, has_my_info)

        print(arts_id)

        #Pobranie ilości stron
        pages = Pages(db.get_db())
        all_pages = pages.get_user_pages_count(arts_id, max_for_page)

        print(all_pages)

        #Pobranie contentu dla wybranej strony
        contents = cont.get_user_contents(arts_id, page, max_for_page)

        #wygenerowanie danych do widoku filtrów

        #wybenerowanie paginacji

        #wygenerowanie contentu
